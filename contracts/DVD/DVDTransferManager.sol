...
contract DVDTransferManager is Ownable {
     ...
     *  @param _encryptedAmountToken1 the amount of `_token1` encrypted that `msg.sender` will send to `_counterpart` at DVD execution time
     ...
     *  @param _encryptedAmountToken2 the amount of `_token2` encrypted that `_counterpart` will send to `msg.sender` at DVD execution time
     ...
     *  @dev initiates a DVD transfer between `msg.sender` & `_counterpart`
     *  @param _token1 the address of the token (ERC20 or TREX) provided by `msg.sender`
     *  @param _token1Amount the amount of `_token1` encrypted that `msg.sender` will send to `_counterpart` at DVD execution time
     *  @param _counterpart the address of the counterpart, which will receive `_token1Amount` of `_token1` in exchange for
     *  `_token2Amount` of `_token2`
     *  @param _token2 the address of the token (ERC20 or TREX) provided by `_counterpart`
     *  @param _token2Amount the amount of `_token2` encrypted that `_counterpart` will send to `msg.sender` at DVD execution time
    ...
    function calculateTransferID(
        ...
        bytes32 transferID = keccak256(
            abi.encode(_nonce, _maker, _token1, _token1Amount, _taker, _token2, _token2Amount)
        );
        return transferID;
    }
}
