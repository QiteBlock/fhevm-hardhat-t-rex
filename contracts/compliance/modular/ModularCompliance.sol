...
contract ModularCompliance is IModularCompliance, OwnableUpgradeable, MCStorage {
     ...
     */
    function transferred(address _from, address _to, euint64 _value) external override onlyToken returns (ebool) {
        require(_from != address(0) && _to != address(0), "invalid argument - zero address");
        uint256 length = _modules.length;
        ebool isModuleTransferActionGood = TFHE.asEbool(true);
        for (uint256 i = 0; i < length; i++) {
            TFHE.allowTransient(_value, _modules[i]);
            isModuleTransferActionGood = TFHE.and(
                isModuleTransferActionGood,
                IModule(_modules[i]).moduleTransferAction(_from, _to, _value)
            );
        }
        TFHE.allowTransient(isModuleTransferActionGood, msg.sender);
        return isModuleTransferActionGood;
    }

    /**
     *  @dev See {IModularCompliance-created}.
     */
    function created(address _to, euint64 _value) external override onlyToken returns (ebool) {
        require(_to != address(0), "invalid argument - zero address");
        // require(_value > 0, 'invalid argument - no value mint');
        uint256 length = _modules.length;
        ebool moduleMinted = TFHE.asEbool(true);
        for (uint256 i = 0; i < length; i++) {
            TFHE.allowTransient(_value, _modules[i]);
            TFHE.allowTransient(moduleMinted, address(_modules[i]));
            moduleMinted = TFHE.and(IModule(_modules[i]).moduleMintAction(_to, _value), moduleMinted);
            TFHE.allowTransient(moduleMinted, address(this));
        }
        TFHE.allowTransient(moduleMinted, msg.sender);
        return moduleMinted;
    }
     ...
     */
    function destroyed(address _from, euint64 _value) external override onlyToken {
    ...
    function canTransfer(address _from, address _to, euint64 _value) external override returns (ebool) {
        ...
        uint256 length = _modules.length;
        ebool checkTransfer = TFHE.asEbool(true);
        for (uint256 i = 0; i < length; i++) {
            TFHE.allowTransient(_value, _modules[i]);
            checkTransfer = TFHE.and(IModule(_modules[i]).moduleCheck(_from, _to, _value, address(this)), checkTransfer);
        }
        TFHE.allowTransient(checkTransfer, msg.sender);
    ...
    function _selector(bytes calldata callData) internal pure returns (bytes4 result) {
        if (callData.length >= 4) {
            ...
            // <https://docs.soliditylang.org/en/v0.7.6/assembly.html#access-to-external-variables-
