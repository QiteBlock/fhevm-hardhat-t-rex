...
contract ConditionalTransferModule is AbstractModuleUpgradeable {
     ...
     */
    event TransferApproved(address _from, address _to, address _token);
     ...
     *  `_token` is the token address of the token concerned by the approval
     */
    event ApprovalRemoved(address _from, address _to, address _token);

    event TransferHash(bytes32 indexed _transferHash);
     ...
     *  @param _amount the array of tokens amounts encrypted that `_from` would send to `_to`
     ...
     *  @param _from the array of addresses of the transfer senders
     *  @param _to the array of addresses of the transfer receivers
     *  @param _amount the array of token amounts encrypted that `_from` were allowed to send to `_to`
     *  Only a bound compliance can call this function
    ...
    /**
     *  Sets the `_encryptedAmount` as the amount of value that has been transferred
    ...
    function moduleTransferAction(
        ...
        return moduleTransferAction(_from, _to, TFHE.asEuint64(_encryptedAmount, _inputProof));
    }
     ...
     */
    function moduleTransferAction(address _from, address _to, euint64 _value) public override onlyComplianceCall returns (ebool) {
        ...
        if (_transfersApproved[msg.sender][transferHash] > 0) {
            ...
            emit ApprovalRemoved(_from, _to, IModularCompliance(msg.sender).getTokenBound());
        }
     ...
     */
    function moduleCheck(address _from, address _to, euint64 _value, address _compliance) public override returns (ebool) {
        ...
        if (isTransferApproved(_compliance, transferHash)) {
            // If transfer is approved return true
            isModuleCheck = TFHE.asEbool(true);
        ...
        // If transfer is not approved return false
        isModuleCheck = TFHE.asEbool(false);
        // Allow the msg.sender to access this cypher in this transaction
        TFHE.allowTransient(isModuleCheck, msg.sender);
        return isModuleCheck;
    }
    ...
    /**
     *  Sets the `_encryptedAmount` as the amount of value to be approved
     */
    function approveTransfer(
        ...
        approveTransfer(_from, _to, TFHE.asEuint64(_encryptedAmount, _inputProof));
    }
     ...
     *  @param _amount the amount of tokens encrypted that `_from` would send to `_to`
     *  Only a bound compliance can call this function
     *  emits a `TransferApproved` event
     */
    function approveTransfer(address _from, address _to, euint64 _amount) public onlyComplianceCall {
        bytes32 transferHash = calculateTransferHash(_from, _to, _amount, IModularCompliance(msg.sender).getTokenBound());
        _transfersApproved[msg.sender][transferHash]++;
        emit TransferApproved(_from, _to, IModularCompliance(msg.sender).getTokenBound());
    }

    /**
     *  Sets the `_encryptedAmount` as the amount of value to be unapproved
     */
    function unApproveTransfer(
        ...
        unApproveTransfer(_from, _to, TFHE.asEuint64(_encryptedAmount, _inputProof));
    }
     ...
     *  @param _from the address of the transfer sender
     *  @param _to the address of the transfer receiver
     *  @param _amount the amount of tokens encrypted that `_from` was allowed to send to `_to`
     *  Only a bound compliance can call this function
     *  emits an `ApprovalRemoved` event
     */
    function unApproveTransfer(address _from, address _to, euint64 _amount) public onlyComplianceCall {
        bytes32 transferHash = calculateTransferHash(_from, _to, _amount, IModularCompliance(msg.sender).getTokenBound());
    ...
    /**
     *  Sets the `_encryptedAmount` as the amount of value to be transferred
     */
    function calculateTransferHash(
        ...
        return calculateTransferHash(_from, _to, TFHE.asEuint64(_encryptedAmount, _inputProof), _token);
    }
     ...
     */
    function calculateTransferHash(address _from, address _to, euint64 _amount, address _token) public returns (bytes32) {
        bytes32 transferHash = keccak256(abi.encode(_from, _to, _amount, _token));
        emit TransferHash(transferHash);
        return transferHash;
    }
