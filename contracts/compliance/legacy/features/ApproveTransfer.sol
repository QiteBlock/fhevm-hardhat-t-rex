...
abstract contract ApproveTransfer is BasicCompliance {
     ...
     */
    event ApprovalRemoved(address _from, address _to, euint64 _amount, address _token);
     ...
     */
    function removeApproval(address _from, address _to, euint64 _amount) external onlyAdmin {
     ...
     */
    function approveAndTransfer(address _from, address _to, euint64 _amount) external {
     ...
     */
    function approveTransfer(address _from, address _to, euint64 _amount) public onlyAdmin {
        bytes32 transferId = _calculateTransferID(_from, _to, _amount, address(tokenBound));
        require(!_transfersApproved[transferId], "transfer already approved");
        _transfersApproved[transferId] = true;
        emit TransferApproved(_from, _to, address(tokenBound));
    }
     ...
     */
    function complianceCheckOnApproveTransfer(address _from, address _to, euint64 _value) public view returns (bool) {
     ...
     */
    function _transferActionOnApproveTransfer(address _from, address _to, euint64 _value) internal {
    ...
    function _creationActionOnApproveTransfer(address _to, uint64 _value) internal {}
     ...
     */
    // solhint-disable-next-line no-empty-blocks
    function _destructionActionOnApproveTransfer(address _from, euint64 _value) internal {}
     ...
     */
    function _transferProcessed(address _from, address _to, euint64 _amount) internal {
        bytes32 transferId = _calculateTransferID(_from, _to, _amount, address(tokenBound));
        if (_transfersApproved[transferId]) {
            _transfersApproved[transferId] = false;
            emit ApprovalRemoved(_from, _to, _amount, address(tokenBound));
