...
abstract contract ExchangeMonthlyLimits is BasicCompliance {
    ...
    mapping(address => euint64) private _exchangeMonthlyLimit;
     ...
     */
    event ExchangeMonthlyLimitUpdated(address _exchangeID, euint64 _newExchangeMonthlyLimit);
     ...
     */
    function setExchangeMonthlyLimit(address _exchangeID, euint64 _newExchangeMonthlyLimit) external onlyOwner {
     ...
     */
    function getMonthlyCounter(address _exchangeID, address _investorID) public view returns (euint64) {
     ...
     */
    function getExchangeMonthlyLimit(address _exchangeID) public view returns (euint64) {
     ...
     */
    function complianceCheckOnExchangeMonthlyLimits(address _from, address _to, euint64 _value) public returns (ebool) {
        ...
        if (!isTokenAgent(_from) && _from != address(0)) {
            if (isExchangeID(receiverIdentity)) {
                ebool isLessThanMonthlyLimit = TFHE.lt(_value, _exchangeMonthlyLimit[receiverIdentity]);
                ebool isLessThanCounter = TFHE.asEbool(true);
                if (!_isExchangeMonthFinished(receiverIdentity, senderIdentity)) {
                    isLessThanCounter = TFHE.lt(
                        TFHE.add(getMonthlyCounter(receiverIdentity, senderIdentity), _value),
                        _exchangeMonthlyLimit[receiverIdentity]
                    );
                }
                return TFHE.and(isLessThanMonthlyLimit, isLessThanCounter);
            }
        }
     ...
     */
    function _transferActionOnExchangeMonthlyLimits(address _from, address _to, euint64 _value) internal {
        address senderIdentity = _getIdentity(_from);
        address receiverIdentity = _getIdentity(_to);
    ...
    function _creationActionOnExchangeMonthlyLimits(address _to, uint64 _value) internal {}
     ...
     */
    // solhint-disable-next-line no-empty-blocks
    function _destructionActionOnExchangeMonthlyLimits(address _from, euint64 _value) internal {}
     ...
     *  internal function, can be called only from the functions of the Compliance smart contract
     */
    function _increaseExchangeCounters(address _exchangeID, address _investorID, euint64 _value) internal {
        _resetExchangeMonthlyCooldown(_exchangeID, _investorID);
        ebool isExchangeMonthlyLimitExceeded = TFHE.le(
            TFHE.add(getMonthlyCounter(_exchangeID, _investorID), _value),
            _exchangeMonthlyLimit[_exchangeID]
        );
        euint64 addValue = TFHE.select(isExchangeMonthlyLimitExceeded, _value, TFHE.asEuint64(0));
        (_exchangeCounters[_exchangeID][_investorID]).monthlyCount = TFHE.add(
            (_exchangeCounters[_exchangeID][_investorID]).monthlyCount,
            addValue
        );
    }
    ...
    function _resetExchangeMonthlyCooldown(address _exchangeID, address _investorID) internal {
        if (_isExchangeMonthFinished(_exchangeID, _investorID)) {
            ...
            (_exchangeCounters[_exchangeID][_investorID]).monthlyCount = TFHE.asEuint64(0);
        }
    }
