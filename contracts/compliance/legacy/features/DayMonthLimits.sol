...
abstract contract DayMonthLimits is BasicCompliance {
     ...
     */
    function setDailyLimit(euint64 _newDailyLimit) external onlyOwner {
     ...
     *  Only the owner of the Compliance smart contract can call this function
     */
    function setMonthlyLimit(euint64 _newMonthlyLimit) external onlyOwner {
     ...
     */
    function complianceCheckOnDayMonthLimits(address _from, address /*_to*/, euint64 _value) public returns (ebool) {
        address senderIdentity = _getIdentity(_from);
        if (!isTokenAgent(_from)) {
            ebool isDailyLimitExceeded = TFHE.le(_value, dailyLimit);
            ebool dailyAndMonthly = TFHE.asEbool(true);
            if (!_isDayFinished(senderIdentity)) {
                ebool isDailyCountExceeded = TFHE.le(
                    TFHE.add(usersCounters[senderIdentity].dailyCount, _value),
                    dailyLimit
                );
                ebool isMonthlyCountExceeded = TFHE.le(
                    TFHE.add(usersCounters[senderIdentity].monthlyCount, _value),
                    monthlyLimit
                );
                dailyAndMonthly = TFHE.and(isDailyCountExceeded, isMonthlyCountExceeded);
            }
            ebool isDailyLimitAndCount = TFHE.and(isDailyLimitExceeded, dailyAndMonthly);
            ebool isMonthFinished = TFHE.asEbool(true);
            if (_isDayFinished(senderIdentity)) {
                bool isMonthFinishedF = _isMonthFinished(senderIdentity);
                ebool isMonthlyCount = TFHE.le(
                    TFHE.add(usersCounters[senderIdentity].monthlyCount, _value),
                    monthlyLimit
                );
                if (isMonthFinishedF) {
                    isMonthFinished = TFHE.or(TFHE.asEbool(true), isMonthlyCount);
                } else {
                    isMonthFinished = TFHE.or(TFHE.asEbool(false), isMonthlyCount);
                }
            }
            return TFHE.and(isDailyLimitAndCount, isMonthFinished);
        }
     ...
     */
    function _transferActionOnDayMonthLimits(address _from, address /*_to*/, euint64 _value) internal {
    ...
    function _creationActionOnDayMonthLimits(address _to, uint64 _value) internal {}
     ...
     */
    // solhint-disable-next-line no-empty-blocks
    function _destructionActionOnDayMonthLimits(address _from, euint64 _value) internal {}
     ...
     */
    function _increaseCounters(address _userAddress, euint64 _value) internal {
        address identity = _getIdentity(_userAddress);
        _resetDailyCooldown(identity);
        _resetMonthlyCooldown(identity);
        ebool isLessThanDailyLimit = TFHE.le(TFHE.add(usersCounters[identity].dailyCount, _value), dailyLimit);
        euint64 addValue = TFHE.select(isLessThanDailyLimit, _value, TFHE.asEuint64(0));
        usersCounters[identity].dailyCount = TFHE.add(usersCounters[identity].dailyCount, addValue);

        ebool isLessThanMonthlyLimit = TFHE.le(TFHE.add(usersCounters[identity].monthlyCount, _value), monthlyLimit);
        euint64 addValueMonth = TFHE.select(isLessThanMonthlyLimit, _value, TFHE.asEuint64(0));
        usersCounters[identity].monthlyCount = TFHE.add(usersCounters[identity].monthlyCount, addValueMonth);
    }
    ...
    function _resetDailyCooldown(address _identity) internal {
        if (_isDayFinished(_identity)) {
            ...
            usersCounters[_identity].dailyCount = TFHE.asEuint64(0);
        }
    }
    ...
    function _resetMonthlyCooldown(address _identity) internal {
        if (_isMonthFinished(_identity)) {
            ...
            usersCounters[_identity].monthlyCount = TFHE.asEuint64(0);
        }
    }
