...
abstract contract MaxBalance is BasicCompliance {
     ...
     */
    function setMaxBalance(euint64 _max) external onlyOwner {
        maxBalance = _max;
     ...
     */
    function complianceCheckOnMaxBalance(address /*_from*/, address _to, euint64 _value) public returns (ebool) {
        ebool isGreaterThanMax = TFHE.le(_value, maxBalance);
        address _id = _getIdentity(_to);
        ebool isIDGreaterThanMax = TFHE.le(TFHE.add(IDBalance[_id], _value), maxBalance);
        return TFHE.and(isGreaterThanMax, isIDGreaterThanMax);
    }
     ...
     */
    function _transferActionOnMaxBalance(address _from, address _to, euint64 _value) internal {
     ...
     *  internal function, can be called only from the functions of the Compliance smart contract
     */
    function _destructionActionOnMaxBalance(address _from, euint64 _value) internal {
        address _idFrom = _getIdentity(_from);
        IDBalance[_idFrom] = TFHE.sub(IDBalance[_idFrom], _value);
    }
}
