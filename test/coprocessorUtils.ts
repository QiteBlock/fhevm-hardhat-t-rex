...
const coprocAdd = parsedEnvCoprocessor.TFHE_EXECUTOR_CONTRACT_ADDRESS.replace(/^0x/, "").replace(/^0+/, "").toLowerCase();
    ...
    switch (selectors[currentSelector]) {
      case "trivialEncrypt(uint256,bytes1)":
        ...
        handle = ethers.keccak256(
          ethers.solidityPacked(["uint8", "uint256", "bytes1"], [Operators.trivialEncrypt, decodedData[0], decodedData[1]])
        );
