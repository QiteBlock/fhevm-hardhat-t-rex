...
export async function deployFullSuiteFixture(ethers: any, signers: Signers, name: string, symbol: string) {
  ...
  const tx = await trexImplementationAuthority.connect(signers.deployer).addAndUseTREXVersion(versionStruct, contractsStruct);
  await tx.wait();
  ...
  const txAdd = await trustedIssuersRegistry.connect(signers.deployer).addTrustedIssuer(claimIssuerContractAddress, claimTopics);
  await txAdd.wait();
  ...
  claimForAlice.signature = await claimIssuerSigningKey.signMessage(
    arrayify(
      ethers.keccak256(
        AbiCoder.encode(["address", "uint256", "bytes"], [claimForAlice.identity, claimForAlice.topic, claimForAlice.data])
      )
    )
  );
...
export async function deploySuiteWithModularCompliancesFixture(ethers: any, trexImplementationAuthorityAddress: string) {
...
export async function deploySuiteWithModuleComplianceBoundToWallet(ethers: any, signers: Signers, name: string, symbol: string) {
