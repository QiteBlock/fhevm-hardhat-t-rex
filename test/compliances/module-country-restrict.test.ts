...
describe("CountryRestrictModule", () => {
  ...
  describe(".canComplianceBind()", () => {
    it("should return true", async () => {
      const context = globalContext;
      expect(await context.suite.countryRestrictModule.canComplianceBind(await context.suite.compliance.getAddress()))
        .to.be.true;
    });
  });
  ...
  describe(".removeCountryRestriction()", () => {
    describe("when the sender is a random wallet", () => {
      it("should reverts", async () => {
        ...
        await expect(
          countryRestrictModule.connect(signers.anotherWallet).removeCountryRestriction(42)
  ...
  describe(".batchRestrictCountries()", () => {
    describe("when the sender is a random wallet", () => {
      it("should reverts", async () => {
        ...
        await expect(
          countryRestrictModule.connect(signers.anotherWallet).batchRestrictCountries([42])
        ).to.be.revertedWith("only bound compliance can call");
      });
    });
  ...
  describe(".batchUnrestrictCountries()", () => {
    describe("when the sender is a random wallet", () => {
      it("should reverts", async () => {
        ...
        await expect(
          countryRestrictModule.connect(signers.anotherWallet).batchUnrestrictCountries([42])
        ).to.be.revertedWith("only bound compliance can call");
      });
    });
