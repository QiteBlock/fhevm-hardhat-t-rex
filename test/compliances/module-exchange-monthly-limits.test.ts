    ...
    describe("when calling via compliance", () => {
      it("should update the limit", async () => {
        ...
        const tx = await context.suite.compliance.callModuleFunction(
          new ethers.Interface(["function setExchangeMonthlyLimit(address _exchangeID, bytes32, bytes)"]).encodeFunctionData(
            "setExchangeMonthlyLimit",
            [exchangeID, encryptedExchangeLimit.handles[0], encryptedExchangeLimit.inputProof]
          ),
    ...
    describe("when calling via compliance", () => {
      describe("when exchangeID is tagged", () => {
        it("should untag the exchangeID", async () => {
          ...
          const tx = context.suite.complianceModule.connect(context.accounts.signers.deployer).removeExchangeID(exchangeID);
