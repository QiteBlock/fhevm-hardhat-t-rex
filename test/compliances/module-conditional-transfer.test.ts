...
describe("ConditionalTransferModule", () => {
  ...
  describe(".canComplianceBind", () => {
    it("should return true", async () => {
      const context = globalContext;
      expect(
        await context.suite.conditionalTransferModule.canComplianceBind(await context.suite.compliance.getAddress())
      ).to.be.true;
    });
  });
  ...
  describe(".batchApproveTransfers", () => {
    ...
    describe("when the sender is the compliance", () => {
      it("should approve the transfers", async () => {
        ...
        expect(
          await conditionalTransferModule.isTransferApproved(await compliance.getAddress(), txReceipt.logs[0].topics[1])
        ).to.be.true;

        expect(
          await conditionalTransferModule.getTransferApprovals(
            await compliance.getAddress(),
      ...
      describe("when the transfer is approved", () => {
        it("should remove the transfer approval", async () => {
          ...
          const tx = await compliance
            ...
            .callModuleFunction(
              new ethers.Interface([
                "function moduleTransferAction(address, address, bytes32, bytes)",
