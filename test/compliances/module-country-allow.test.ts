  ...
  describe(".batchAllowCountries()", () => {
    describe("when calling not via the Compliance contract", () => {
      it("should revert", async () => {
        ...
        await expect(countryAllowModule.connect(signers.anotherWallet).batchAllowCountries([42, 66])).to.be.revertedWith(
  ...
  describe(".batchDisallowCountries()", () => {
    describe("when calling not via the Compliance contract", () => {
      it("should revert", async () => {
        ...
        await expect(countryAllowModule.connect(signers.anotherWallet).batchDisallowCountries([42, 66])).to.be.revertedWith(
          "only bound compliance can call"
        );
      });
    });
        ...
        it("should revert", async () => {
          ...
          await expect(
            compliance
              .connect(signers.deployer)
              .callModuleFunction(
                new ethers.Interface(["function addAllowedCountry(uint16 country)"]).encodeFunctionData("addAllowedCountry", [
                  42,
                ]),
                await countryAllowModule.getAddress()
              )
      ...
      describe("when country is allowed", () => {
        it("should disallow the given country", async () => {
          ...
          const tx = await compliance
            .connect(signers.deployer)
            .callModuleFunction(
              new ethers.Interface(["function removeAllowedCountry(uint16 country)"]).encodeFunctionData("removeAllowedCountry", [
                42,
              ]),
              await countryAllowModule.getAddress()
            );
