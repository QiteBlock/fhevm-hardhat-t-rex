...
describe("Token - Transfers", () => {
  ...
  describe(".batchTransfer()", () => {
    it("should transfer tokens", async () => {
      ...
      const inputAlice2 = instances.aliceWallet.createEncryptedInput(
        await token.getAddress(),
        signers.aliceWallet.address
      );
      let addresses = [signers.bobWallet.address, signers.bobWallet.address];
    ...
    describe("when sender is not an agent", () => {
      it("should revert", async () => {
        ...
        try {
          await token
            .connect(signers.aliceWallet)
            .forcedTransfer(signers.aliceWallet.address, signers.bobWallet.address, 100);
        } catch (error) {
          expect(error.message).to.include("AgentRole: caller does not have the Agent role");
        }
      });
