      ...
      describe("when identity is zero address", () => {
        it("should revert", async () => {
          ...
          await expect(
            identityRegistryStorage
              .connect(signers.tokenAgent)
              .modifyStoredIdentity(signers.charlieWallet.address, ZERO_ADDRESS)
          ).to.be.revertedWith("invalid argument - zero address");
        });
      });
    ...
    describe("when sender is not agent", () => {
      it("should revert", async () => {
        ...
        await expect(
          identityRegistryStorage
            .connect(signers.anotherWallet)
            .modifyStoredInvestorCountry(signers.charlieWallet.address, 42)
        ).to.be.revertedWith("AgentRole: caller does not have the Agent role");
      });
    });
    ...
    describe("when sender is not agent", () => {
      it("should revert", async () => {
        ...
        await expect(
          identityRegistryStorage
            .connect(signers.anotherWallet)
            .removeIdentityFromStorage(signers.charlieWallet.address)
        ).to.be.revertedWith("AgentRole: caller does not have the Agent role");
      });
    });
    ...
    describe("when sender is owner", () => {
      describe("when identity registries is zero address", () => {
        it("should revert", async () => {
          ...
          await expect(
            identityRegistryStorage.connect(signers.deployer).bindIdentityRegistry(ZERO_ADDRESS)
          ).to.be.revertedWith("invalid argument - zero address");
        });
      });
    ...
    describe("when sender is agent", () => {
      describe("when identity registries is zero address", () => {
        it("should revert", async () => {
          ...
          await expect(
            identityRegistryStorage.connect(signers.deployer).unbindIdentityRegistry(ZERO_ADDRESS)
          ).to.be.revertedWith("invalid argument - zero address");
        });
      });
