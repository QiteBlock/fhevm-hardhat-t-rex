    ...
    describe("when sender is the owner", () => {
      describe("when issuer to add is zero address", () => {
        it("should revert", async () => {
          ...
          await expect(
            trustedIssuersRegistry.connect(signers.deployer).addTrustedIssuer(ZERO_ADDRESS, [10])
      ...
      describe("when issuer is already registered", () => {
        it("should revert", async () => {
          ...
          const claimTopics = await trustedIssuersRegistry.getTrustedIssuerClaimTopics(
    ...
    describe("when sender is the owner", () => {
      describe("when issuer to remove is zero address", () => {
        it("should revert", async () => {
          ...
          await expect(
            trustedIssuersRegistry.connect(signers.deployer).removeTrustedIssuer(ZERO_ADDRESS)
          ).to.be.revertedWith("invalid argument - zero address");
        });
      });
      ...
      describe("when issuer is registered", () => {
        it("should remove the issuer from trusted list", async () => {
          ...
          const tx = await trustedIssuersRegistry
            .connect(signers.deployer)
            .removeTrustedIssuer(signers.anotherWallet.address);
          await expect(tx)
            .to.emit(trustedIssuersRegistry, "TrustedIssuerRemoved")
            .withArgs(signers.anotherWallet.address);
      ...
      it("should revert", async () => {
        ...
        await expect(
          trustedIssuersRegistry
            ...
            .updateIssuerClaimTopics(signers.anotherWallet.address, [10])
        ).to.be.revertedWith("Ownable: caller is not the owner");
      });
    });
    ...
    describe("when sender is the owner", () => {
      ...
      describe("when claim topics array is empty", () => {
        it("should revert", async () => {
          ...
          await expect(
            trustedIssuersRegistry
              .connect(signers.deployer)
              .updateIssuerClaimTopics(await claimIssuerContract.getAddress(), [])
      ...
      describe("when issuer is registered", () => {
        it("should update the topics of the trusted issuers", async () => {
          ...
          expect(await trustedIssuersRegistry.hasClaimTopic(await claimIssuerContract.getAddress(), claimTopics[0])).to
            .be.false;
          expect(
            await trustedIssuersRegistry.getTrustedIssuerClaimTopics(await claimIssuerContract.getAddress())
          ).to.deep.eq([66, 100]);
          await ethers.provider.send("evm_revert", [snapshotId]);
        });
      });
