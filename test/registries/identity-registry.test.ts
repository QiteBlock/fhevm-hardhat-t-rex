...
describe("IdentityRegistry Test", () => {
  ...
  describe(".init()", () => {
    it("should prevent re-initialization", async () => {
      ...
      await expect(
        identityRegistry.connect(signers.deployer).init(ZERO_ADDRESS, ZERO_ADDRESS, ZERO_ADDRESS)
    ...
    it("should reject zero address for Trusted Issuers Registry", async () => {
      ...
      await expect(identityRegistry.init(ZERO_ADDRESS, address, address)).to.be.revertedWith(
    ...
    it("should reject zero address for Claim Topics Registry", async () => {
      ...
      await identityRegistry.waitForDeployment();
      const address = ethers.Wallet.createRandom().address;
      await expect(identityRegistry.init(address, ZERO_ADDRESS, address)).to.be.revertedWith(
        "invalid argument - zero address"
      );
    });
    ...
    it("should reject zero address for Identity Storage", async () => {
      ...
      await identityRegistry.waitForDeployment();
      const address = ethers.Wallet.createRandom().address;
      await expect(identityRegistry.init(address, address, ZERO_ADDRESS)).to.be.revertedWith(
        "invalid argument - zero address"
      );
    });
  });
  ...
  describe(".setIdentityRegistryStorage()", () => {
    describe("when sender is not the owner", () => {
      it("should revert", async () => {
        ...
        await expect(
          identityRegistry.connect(signers.anotherWallet).setIdentityRegistryStorage(ZERO_ADDRESS)
  ...
  describe(".setClaimTopicsRegistry()", () => {
    describe("when sender is not the owner", () => {
      it("should revert", async () => {
        ...
        await expect(
          identityRegistry.connect(signers.anotherWallet).setClaimTopicsRegistry(ZERO_ADDRESS)
        ).to.be.revertedWith("Ownable: caller is not the owner");
      });
    });
  ...
  describe(".setTrustedIssuersRegistry()", () => {
    describe("when sender is not the owner", () => {
      it("should revert", async () => {
        ...
        await expect(
          identityRegistry.connect(signers.anotherWallet).setTrustedIssuersRegistry(ZERO_ADDRESS)
        ).to.be.revertedWith("Ownable: caller is not the owner");
      });
    });
    ...
    describe("when there are no require claim topics", () => {
      it("should return true when the identity is registered", async () => {
        ...
        await identityRegistry
          .connect(signers.tokenAgent)
          .registerIdentity(signers.charlieWallet.address, charlieIdentity, 0);
