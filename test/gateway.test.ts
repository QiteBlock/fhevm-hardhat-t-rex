  ...
  describe(".setFactory()", () => {
    describe("when called by not owner", () => {
      it("should revert", async () => {
        ...
        const gateway = await ethers.deployContract("TREXGateway", [ZERO_ADDRESS, false], context.accounts.signers.deployer);
        await context.factories.trexFactory
          .connect(context.accounts.signers.deployer)
          .transferOwnership(await gateway.getAddress());

        await expect(
          gateway.connect(context.accounts.signers.anotherWallet).setFactory(await context.factories.trexFactory.getAddress())
        ).to.be.reverted;
  ...
  describe(".setPublicDeploymentStatus()", () => {
    describe("when called by not owner", () => {
      it("should revert", async () => {
        ...
        await expect(gateway.connect(context.accounts.signers.anotherWallet).setPublicDeploymentStatus(true)).to.be.reverted;
        await ethers.provider.send("evm_revert", [snapshotId]);
      });
    });
    describe("when called by owner", () => {
    ...
    describe("when called by not owner", () => {
      it("should revert", async () => {
        ...
        await expect(
          gateway
            .connect(context.accounts.signers.anotherWallet)
            .setDeploymentFee(100, await context.suite.token.getAddress(), context.accounts.signers.anotherWallet.address)
        ).to.be.reverted;
        await ethers.provider.send("evm_revert", [snapshotId]);
      });
    });
  ...
  describe(".addDeployer()", () => {
    describe("when called by not admin", () => {
      it("should revert", async () => {
        ...
        await expect(
          gateway.connect(context.accounts.signers.anotherWallet).addDeployer(context.accounts.signers.anotherWallet.address)
    ...
    describe("when called by agent", () => {
      describe("if deployer already exists", () => {
        it("should revert", async () => {
          ...
          await gateway.addDeployer(context.accounts.signers.tokenAgent.address);
          await expect(
            gateway.connect(context.accounts.signers.tokenAgent).addDeployer(context.accounts.signers.tokenAgent.address)
    ...
    describe("when called by agent", () => {
      describe("when adding a batch of deployers that includes an already registered deployer", () => {
        it("should revert", async () => {
          ...
          await gateway.connect(context.accounts.signers.anotherWallet).addDeployer(context.accounts.signers.tokenAgent.address);
          const newDeployers = Array.from({ length: 9 }, () => ethers.Wallet.createRandom().address);
          const randomIndex = Math.floor(Math.random() * newDeployers.length);
          newDeployers.splice(randomIndex, 0, context.accounts.signers.tokenAgent.address);
          await expect(
  ...
  describe(".removeDeployer()", () => {
    describe("when called by not owner", () => {
      it("should revert", async () => {
        ...
        await expect(
          gateway.connect(context.accounts.signers.anotherWallet).removeDeployer(context.accounts.signers.anotherWallet.address)
        ).to.be.revertedWithCustomError(gateway, "OnlyAdminCall");
        await ethers.provider.send("evm_revert", [snapshotId]);
      });
    });
    describe("when called by owner", () => {
      describe("if deployer does not exist", () => {
        it("should revert", async () => {
          ...
          await expect(gateway.removeDeployer(context.accounts.signers.tokenAgent.address)).to.be.revertedWithCustomError(
            gateway,
    ...
    describe("when called by owner", () => {
      describe("if deployer does not exist", () => {
        it("should revert", async () => {
          ...
          await expect(gateway.batchRemoveDeployer([context.accounts.signers.tokenAgent.address])).to.be.revertedWithCustomError(
            gateway,
            "DeployerDoesNotExist"
          );
          await ethers.provider.send("evm_revert", [snapshotId]);
    ...
    describe("when called by agent", () => {
      ...
      describe("if all discounts are valid", () => {
        it("should apply discounts to all deployers", async () => {
          ...
          await gateway.addAgent(context.accounts.signers.tokenAgent.address);
          const tx = await gateway.connect(context.accounts.signers.tokenAgent).batchApplyFeeDiscount(deployers, discounts);
        ...
        describe("when deployment fees are activated", () => {
          describe("when caller has no discount", () => {
            it("should deploy a token for full fee", async () => {
              ...
              await gateway.setDeploymentFee(20000, await feeToken.getAddress(), context.accounts.signers.deployer.address);
              await gateway.enableDeploymentFee(true);
    ...
    describe("when called by not deployer", () => {
      describe("when public deployments disabled", () => {
        it("should revert for batch deployment", async () => {
          ...
          await expect(
            gateway.connect(context.accounts.signers.anotherWallet).batchDeployTREXSuite(tokenDetailsArray, claimDetailsArray)
          ).to.be.revertedWithCustomError(gateway, "PublicDeploymentsNotAllowed");
          await ethers.provider.send("evm_revert", [snapshotId]);
        });
      });
        ...
        describe("when deployment fees are activated", () => {
          describe("when caller has no discount", () => {
            it("should deploy tokens for full fee in a batch", async () => {
              ...
              await feeToken.connect(context.accounts.signers.anotherWallet).approve(await gateway.getAddress(), 100000);
              ...
              for (let i = 0; i < tokenDetailsArray.length; i += 1) {
                ...
                expect(tx)
                  ...
                  .withArgs(context.accounts.signers.anotherWallet.address, context.accounts.signers.deployer.address, 20000);
              }
          ...
          describe("when caller has 50% discount", () => {
            it("should deploy tokens for half fee in a batch", async () => {
              ...
              for (let i = 0; i < tokenDetailsArray.length; i += 1) {
                ...
                expect(tx)
                  .to.emit(feeToken, "Transfer")
                  .withArgs(context.accounts.signers.anotherWallet.address, context.accounts.signers.deployer.address, 10000);
              }
