  ...
  describe(".recoverContractOwnership()", () => {
    describe("when sender is not owner", () => {
      it("should revert", async () => {
        ...
        const receipt = await tx.wait();
        let tokenAddressExpected = ethers.hexlify(receipt.logs[43].topics[1]);
        const tokenAddress = "0x" + tokenAddressExpected.slice(26, tokenAddressExpected.length);
        await expect(
          trexFactory.connect(signers.aliceWallet).recoverContractOwnership(tokenAddress, signers.aliceWallet.address)
        ).to.be.reverted;
        await ethers.provider.send("evm_revert", [snapshotId]);
      });
    });
